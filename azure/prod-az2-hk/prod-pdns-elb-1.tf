module "prod-pdns-elb-1" {
  source = "../modules/app"

  resource_group_name = local.resource_group_name

  load_balancer = {
    name                    = "prod-pdns-elb-1"
    frontend_config_name    = "prod-pdns-elb-frontend-1"
    frontend_public_ip_name = "prod-pdns-elb-frontend-1-ip"
    backend_pool_name       = "prod-pdns-elb-1"
  }

  vmss = {
    name            = "prod-pdns-vmss-1"
    hostname_prefix = "prod-az2-hk-pdns1"
    instance_count  = 1
    capacity = {
      min = 1
      max = 16
